...
ltt.df.HPD <- function(phy){
    ...
    mean_height = as.numeric(phy$metadata$height[root:dim(phy$metadata)[1]])
    ...
    node_height = data.frame(mean=mean_height, lower=hpd_height[,1], upper=hpd_height[,2])
    node_height = -node_height[ order(node_height[,1], decreasing=T), ]
